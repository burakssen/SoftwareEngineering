...
const {BAD_REQUEST, NOT_AUTHORIZED} = require("../common/constants/statusCodes");
const {JWT_SECRET_KEY} = require("../common/constants/secretKeys");
const {getEmployeeService} = require("../serviceCallers/employee");
...
exports.createVideoToken = (payload) => {
    return sign(
        {videoId: payload.videoId, employeeId: payload.id},
        JWT_SECRET_KEY
    );
};
...
exports.validateToken = async (req, res, next) => {
    ...
    if (!accessToken)
        return res.status(NOT_AUTHORIZED).json({error: "User not Authenticated!"});
    try {
        if (verify(accessToken, JWT_SECRET_KEY)) {
            ...
            return res.status(NOT_AUTHORIZED).json({error: "Invalid token!"});
        }
    } catch (err) {
...
exports.adminAuth = (req, res, next) => {
    const request_role = req.role;
    if (request_role !== ADMIN) {
        return res.status(NOT_AUTHORIZED).json({error: "Access denied"});
    }
    next();
};
